...
import { useAuth } from "../context/AuthContext.jsx"; // To restrict access
...
const CreateJobPage = () => {
  ...
  // Redirect if not authorized after authLoading is complete
  if (
    !authLoading &&
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Convert comma-separated requirements string to an array
      ...
      // Remove empty application fields if only one is provided for backend validation
      if (payload.applicationLink && !payload.applicationEmail) {
        delete payload.applicationEmail;
      } else if (payload.applicationEmail && !payload.applicationLink) {
        delete payload.applicationLink;
      ...
      navigate(`/jobs/${response.data.data._id}`); // Go to new job's detail page
    } catch (err) {
      const errorMessage =
        err.response?.data?.message ||
        "Failed to create job listing. Please check your input.";
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          >
            ...
            <option value="Temporary">Hybrid</option>
          </select>
        </div>
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            List each requirement, separated by a comma (e.g., "Skill 1, Skill
            2, Skill 3")
          </p>
        </div>
        ...
        <div className="border-t pt-6 mt-6">
          <h3 className="text-xl font-semibold text-gray-800 mb-4">
            Application Method (Choose one or both)
          </h3>
          <div>
            <label
          ...
          {error && error.includes("application") && (
            <p className="mt-2 text-sm text-red-600">{error}</p>
          )}
        </div>

        {error && (
          <p className="mt-4 text-center text-red-600 text-md">{error}</p>
        )}
        {success && (
          <p className="mt-4 text-center text-green-600 text-md">{success}</p>
        )}
